...
describe('useEmojiBroadcast', () => {
  ...
  it('ignora lastEmoji com sentAt anterior ao clientMountAt', async () => {
    ...
    const [e, cleanup] = withSetup(() => useEmojiBroadcast({
      room,
      myUid: ref('alice'),
      roomId: ref('r'),
    }))
  ...
  it('clearBubble remove o slot do uid', async () => {
    const room = ref<Room | null>(roomWith({}))
    const [e, cleanup] = withSetup(() => useEmojiBroadcast({ room, myUid: ref('alice'), roomId: ref('r') }))
    await nextTick()
    room.value = roomWith({ alice: { lastEmoji: { value: '🎉', sentAt: Timestamp.fromMillis(Date.now() + 100) } } })
    await nextTick()
    e.clearBubble('alice')
  ...
  it('sendEmoji aceita emoji com variation selector (ex: ❤️)', async () => {
    const { sendEmoji: svc } = await import('@/services/firebase/rooms')
    const room = ref<Room | null>(roomWith({}))
    const [e, cleanup] = withSetup(() => useEmojiBroadcast({ room, myUid: ref('alice'), roomId: ref('r') }))
    await e.sendEmoji('❤️')
    expect(svc).toHaveBeenCalledWith('r', 'alice', '❤️')
    cleanup()
  })
})
